...
def main():
    ...
    # checkpoint.save_checkpoint(model, optimizer, lr_sched, loss_scaler, i + 1, args)
    print(f'Total training time: {datetime.now() - train_st}, Average train time:{(datetime.now() - train_st) /len(train_loader)}')
...
def evaluate(model: torch.nn.Module, loader: torch.utils.data.DataLoader):
    ...
    print(f'Total validation time: {datetime.now() - eval_st}, Average val. time:{(datetime.now() - eval_st) /len(loader)}')
